...
我们会在ptr<undef>, Ptr<undef>里使用undef类型, 因为指针是可以索引值的, 而void不具备一个值, 索引我们使用undef, undef会在llvm中生成为i8类型.

Prajna是强类型语言, 不同类型是不可以相互赋值的, 你需要明确的调用类型转换函数. 除此之外, 我们还会自定义结构类型类型, 这我们在后面的章节再做介绍

### function类型

function<arg_type0, arg_type1, return_type> 本质上是一个函数指针类型, 但我们直接用function来表示. 我们不会用到function的值类型, 所以我们没有提供直接的函数类型
...
我们可以看到Prajna里的文件库就两个函数Write和Read.
