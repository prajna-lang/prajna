        ...
        for (auto ir_call : ir_calls) {
            ...
            for (auto ir_return : ir_returns) {
                if (Is<ir::VoidValue>(ir_return->Value())) {
                    ...
                    auto scope_1 = ir_return_builder->PushBlockRAII(ir_return->GetParentBlock());
            ...
            ir_new_callee->blocks.front()->parent.reset();
            ir_call->GetParentBlock()->Insert(iter, ir_new_callee->blocks.front());
