...
我们也可以通过<https://github.com/prajna-lang/prajna/releases>直接获取主要平台的二进制程序, 然后将bin目录加入到可执行路径里.
如果没有对应平台的二进制程序, 我们可以从源码编译安装

#### Windows环境配置

我们需要在windows平台下安装

* visual studio, 需要包含(C++模块)
* cmake
* python
* ninja-build
* git, git lfs和git-bash 安装git时候注意勾选

#### Ubuntu环境配置

可以直接使用docker的环境来编译代码, 也可以自行参阅[dockerfile](../dockerfiles/ubuntu_dev.dockerfile)来配置环境.
下文会有更具体的介绍

#### 下载源码

首先我们下载源码, 下载的库会比较多, 如果没有报错请耐心等待, 建议配置网络代理以便能流畅下载github的代码,
对于bash脚本和指令, windows平台我们应该进入git-bash里执行

```bash
# 下载代码
git clone https://github.com/matazure/prajna.git
# 下载依赖库
bash ./scripts/clone_submodules.sh --jobs=16 --depth=100
```
...
```bash
bash ./scritps/clone_submodules.sh --force --jobs=16
```

在代码下载中如果出现网络错误, 请自行配置VPN.

#### 编译

#### 环境配置

* 如果使用docker环境, 则配置好docker后直接使用该指令即可, 无需配置环境.
...
* 如果是windows平台, 则需要我们执行

```bat
call C:\Program Files\Microsoft Visual Studio\2022\Community\VC\Auxiliary\Build\vcvars64.bat
```

这样我们才能正确获取C++的编译器配置, 在编译完之前无退出该环境.

#### 构建和编译

```bash
bash ./scripts/configure.sh release # 配置为release模式
...
```bash
bash ./scripts/test.sh release # 我们可以通过改指令来运行测试, 这是非必须的步骤
```
...
### \_\_initialize\_\_, \_\_copy\_\_ 和 \_\_finalize\_\_
