...
与 OpenCL、CUDA 只是对 C++ 的扩展不同，Prajna 原生支持（基于核函数的）并行编程，对 GPGPU 的支持比 CUDA 更加友好，并且未来将支持更多 GPGPU（目前仅支持 Nvidia）。

#### 内置张量优化编译器

后续版本将从语言层面集成自研的张量优化（暂称波罗蜜多）编译器，为张量计算提供极具竞争力的解决方案，并兼容多种 NPU/TPU。
