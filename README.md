...
般若编译器(Prajna Compiler)是新一代的专注于人工智能领域的编译平台. 般若编译器不是要做一个取代Python或者C++这种通用编程语言的编译器, 其只会专注于人工智能基础设施领域. 般若希望可以改善人工智能领域的编程环境, 构建新一代的人工智能基础设施, 推动人工智能的创新. 下面会阐述般若编译器的一些特点.
...
```
上面是一段gpu的代码, 该代码和CUDA是高度相似的. 般若并非要做一个国产版的CUDA, 般若追求更加自然和统一的异构计算, 类似CUDA的特性只是实现这一目标的必经之路.
...
目前般若编译器团队只有我一人, 虽然我奉行"尽可能简单"的原则, 但在短时间内去做这样一个编译器是不现实的. 故在此寻求天使投资, 以便组建团队, 有意联系.
...
招聘实习生一位, 本科及其以上学历, 具备一定的编程和数学基础即可, 有意联系
